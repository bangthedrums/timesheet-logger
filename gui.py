...
#Background colours
main_win_bg = "#61C4E9" # Main GUI window
manproj_win_bg = "#61C4E9" # Manage projects window
summary_win_bg = "#61C4E9" # View Summary window
edit_win_bg = "#61C4E9" # Edit log window
...
def open_manage_projects(projects):
    ...
    tk.Label(manage_win, text="Edit Project Names:", bg=manproj_win_bg).pack(pady=5)
...
def show_summary_window():
    ...
    win.configure(bg=summary_win_bg)
    win.attributes("-topmost", True)

    # === Scrollable Canvas Setup ===
    canvas = tk.Canvas(win, bg=summary_win_bg, height=400)
    ...
    win.geometry("")  # Let the window auto-size cleanly to its contents
...
def open_edit_log_window():
    ...
    win = tk.Toplevel(popup)
    win.title("Edit Today's Log")
    win.geometry("")
    win.configure(bg=edit_win_bg)
    win.attributes("-topmost", True)

    tk.Label(win, text="Time booked today:", bg=edit_win_bg, font=("Arial", 10, "bold")).pack(pady=(10, 5))

    table_frame = tk.Frame(win, bg=edit_win_bg)  # black grid lines
    ...
    # FROM project
    tk.Label(win, text="Move Time FROM:", bg=edit_win_bg).pack()
    from_var = tk.StringVar()
    from_menu = ttk.Combobox(win, textvariable=from_var, values=list(totals.keys()), state="readonly")
    from_menu.pack()

    # TO project
    tk.Label(win, text="Move Time TO:", bg=edit_win_bg).pack()
    to_var = tk.StringVar()
    all_projects = load_projects()
    to_menu = ttk.Combobox(win, textvariable=to_var, values=all_projects, state="readonly")
    to_menu.pack()

    # Amount to move
    tk.Label(win, text="Hours to move:", bg=edit_win_bg).pack()
    hours_var = tk.StringVar()
    tk.Entry(win, textvariable=hours_var).pack(pady=5)

    msg_label = tk.Label(win, text="", fg="red", bg=edit_win_bg)
    msg_label.pack()
    ...
    def apply_adjustment():
        ...
        win.destroy()
        show_popup(load_projects())  # refresh UI

    # Buttons
    btn_frame = tk.Frame(win, bg=edit_win_bg)
    btn_frame.pack(pady=10)

    tk.Button(btn_frame, text="Apply", command=apply_adjustment, width=10).grid(row=0, column=0, padx=5)
    tk.Button(btn_frame, text="Cancel", command=win.destroy, width=10).grid(row=0, column=1, padx=5)
...
def show_popup(projects, reschedule_callback=None):
    ...
    status = f"Last logged: {current_project or '-'}"
